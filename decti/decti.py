    ...
    def predict(self, image, batch_size=0, device='cpu'):
        ...
        with torch.no_grad():
            ...
            for i_in_img in range(nbatch_per_img):
                ...
                if i_in_img == 0:
                    output = torch.squeeze(batch_output, axis=-1)
                else:
                    output = torch.cat([output, torch.squeeze(batch_output, axis=-1)], axis=1)

        output = output.to('cpu').to_numpy()
        output = self.data_manager.post_process(output)
    ...
    def batch_predict(
        ...
        try:
            ...
            if not dist.is_initialized():
                raise Exception('process group not properly initialized at rank {}'.format(rank))

            # assign model to device
            device = torch.device('cuda:{}'.format(local_rank)) if use_gpu else torch.device('cpu')
            ddp_model = self.model.to(device)
            dist.barrier()
            ddp_model = DDP(ddp_model, device_ids=[local_rank])
            ddp_model.eval()

            # load data
            dist.barrier()
            _, ev_loader, _ = load_data(
                self.data_manager, input_paths, output_paths, train=False, num_workers=loader_workers)
                ...
                for idx, (img_input, _, _, _, path_output) in enumerate(ev_loader):
                    ...
                    for i_in_img in range(nbatch_per_img):
                        ...
                        if i_in_img == 0:
                            ...
                            img_output = torch.cat([img_output, torch.squeeze(batch_output, dim=-1)], dim=1)

                    img_output = self.data_manager.post_process(img_output.to('cpu').numpy())
                    self.data_manager.write(img_output, path_output[0])
                    if verbose and rank == 0:
                        print('image {}/{} processed at rank {}'.format(idx+1, len(ev_loader), rank))
                        sys.stdout.flush()
